...
import React, { useState } from "react";
import { useNavigate } from "react-router-dom";
import { GoogleLogin } from "@react-oauth/google"; // ID Token එක සඳහා GoogleLogin භාවිතා කරමු
...
const Register = () => {
  ...
  const handleGoogleSuccess = async (credentialResponse) => {
    setIsSubmitting(true);
    try {
      // Backend එකට ID Token එක යවනවා (Backend logic එකෙන් user ව auto-create කරනවා)
      ...
      toast.success(
        `Authenticated as ${response.role}! Now complete your profile.`,
      );

      // Profile details ලබාගන්න Modal එක open කරනවා
      if (role === "candidate") setIsCandidateModalOpen(true);
      else setIsInterviewerModalOpen(true);
    } catch (error) {
  ...
  const handleCandidateComplete = async (candidateExtraData, imageFile) => {
    setIsSubmitting(true);
    try {
      // 🎯 1. Google එකෙන් ආවා නම් දැනටමත් Authenticated. නැත්නම් විතරක් Register/Login කරමු.
      ...
      await CandidateService.completeProfile(payload, imageFile);
      setIsCandidateModalOpen(false);
      toast.success("Welcome to ColloQ!");
      navigate("/dashboard/candidate");
    } catch (error) {
      toast.error(
  ...
  const handleInterviewerComplete = async (interviewerExtraData, imageFile) => {
    ...
    try {
      ...
      const payload = {
        bio: interviewerExtraData.bio,
        company: interviewerExtraData.company,
        levelId: parseInt(interviewerExtraData.levelId),
        experienceYears: parseInt(interviewerExtraData.experienceYears),
        githubUrl: interviewerExtraData.githubUrl,
        linkedinUrl: interviewerExtraData.linkedinUrl,
        specializations: interviewerExtraData.specializations,
        status: "PENDING",
      };
      ...
      toast.error(error.response?.data?.message || "Interviewer setup failed.");
    } finally {
      setIsSubmitting(false);
    }
  };
        ...
        <div className="w-full max-w-[440px]">
          ...
          <form onSubmit={handleInitialRegister} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={isSubmitting}
              className="w-full py-4 mt-4 rounded-sm font-black text-white bg-orange-600 hover:bg-orange-500 transition-all uppercase tracking-[0.2em] text-xs disabled:opacity-50"
            >
              {isSubmitting ? "PROCESSING..." : `CONTINUE AS ${role}`}
            </button>
          </form>
