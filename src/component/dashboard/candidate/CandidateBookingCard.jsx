...
const CandidateBookingCard = ({ booking, onPay }) => {
  // Status අනුව පාට තීරණය කිරීම (Glass look එකට ගැලපෙන සේ)
  ...
  return (
    <div className="relative overflow-hidden border border-white/10 bg-white/[0.03] backdrop-blur-md p-5 rounded-xl flex flex-col gap-4 group hover:bg-white/[0.05] hover:border-orange-500/40 transition-all duration-500 shadow-2xl">
      ...
      {/* Header: Interviewer Info & Status */}
      ...
      {/* Action Buttons Logic (Modern Buttons) */}
        ...
        {booking.status === "APPROVED" ? (
          booking.isPaid ? (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="w-full py-2.5 bg-green-500/20 hover:bg-green-500 text-green-400 hover:text-white border border-green-500/30 text-[9px] font-bold uppercase tracking-[0.1em] rounded-lg flex items-center justify-center gap-2 transition-all active:scale-95 shadow-lg"
            >
              <VideoCallIcon sx={{ fontSize: 16 }} /> Join Session
            </a>
          ) : (
            <button
          ...
          <div className="w-full py-2.5 bg-white/5 border border-white/5 text-gray-500 text-[8px] font-bold uppercase tracking-[0.15em] text-center rounded-lg backdrop-blur-sm">
