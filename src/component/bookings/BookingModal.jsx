...
const InterviewerSelectionModal = ({ isOpen, onClose }) => {
  ...
  const [isBookingLoading, setIsBookingLoading] = useState(false); // 👈 අලුතින්
  ...
  const handleConfirmBooking = async () => {
    ...
    setIsBookingLoading(true); // 👈 Loading Start
    try {
      const bookingData = {
        interviewerId: selectedInterviewer.interviewerId,
        availabilityId: selectedSlot.availabilityId,
        levelId: selectedInterviewer.levelId, // 👈 Level ID එකත් යවන්න
        status: "PENDING",
        // 💡 Candidate ID එක සාමාන්‍යයෙන් Backend එකේදී Token එකෙන් ගනියි.
        // නැත්නම් localStorage එකේ තියෙනවා නම් මෙතනින් යවන්න.
      };
      ...
      setIsBookingLoading(false); // 👈 Loading Stop
    }
  };
  ...
  const filteredInterviewers = interviewers.filter((interviewer) => {
    const matchesSpec =
      ...
      interviewer.specialization
        ?.toUpperCase()
        .includes(
          activeFilters.specialization.toUpperCase().replace(/\s+/g, ""),
        );
    const matchesLevel =
          ...
          <div className="flex-1 flex flex-col items-center justify-center bg-[#080808] p-12 overflow-y-auto custom-scrollbar animate-in slide-in-from-right duration-500">
            <div className="w-full max-w-2xl space-y-10">
              ...
              <div className="space-y-2">
                ...
                <p className="text-gray-500 text-[11px] font-medium italic tracking-wide">
                  Booking with{" "}
                  <span className="text-white font-bold underline decoration-orange-500/50">
              ...
              {/* Price Box */}
              <div className="border-2 border-dashed border-white/10 rounded-2xl p-10 bg-white/5 flex flex-col items-center justify-center space-y-2 hover:border-orange-500/40 transition-all duration-500">
              ...
              <div className="space-y-6">
                ...
                {isSlotsLoading ? (
                  <p className="text-[10px] text-gray-600 animate-pulse uppercase font-black">
                    Finding slots...
                  </p>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {availabilitySlots.map((slot) => (
                      <button
                        ...
                        className={`p-4 rounded-xl text-[10px] text-left font-black transition-all border flex flex-col gap-1 ${selectedSlot?.availabilityId === slot.availabilityId ? "bg-orange-600 border-orange-600 text-white shadow-xl scale-[1.02]" : "bg-white/5 border-white/10 text-gray-400 hover:border-white/30"}`}
                      >
              ...
              <button
                ...
                className={`w-full py-5 rounded-xl text-[12px] font-black uppercase tracking-[0.3em] transition-all duration-300 ${!selectedSlot || isBookingLoading ? "bg-white/5 text-gray-700 cursor-not-allowed border border-white/5" : "bg-white text-black hover:bg-orange-600 hover:text-white shadow-xl active:scale-95"}`}
              >
                {isBookingLoading
                  ? "Processing Request..."
                  : selectedSlot
